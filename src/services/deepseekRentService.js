...
${offerProspectNote?.trim() ? `
【用户主观岗位前景评估】
${offerProspectNote.trim()}

注意：上述主观评估由用户基于与 HR / 业务主管沟通后填写，请认真对待。若该描述显示岗位前景极好（如核心项目、明确晋升路径、优质团队），即使薪资数字略低，也应适当增加推荐该 Offer 的权重，并在报告中明确说明前景对判断的加成作用。` : ''}

请按以下结构输出分析报告（语言简洁专业，直接给建议，避免废话；请同时结合薪资数据与岗位前景综合判断，给出最符合长远职业发展的建议）：
...
`.trim()
  ...
  const response = await fetch(DEEPSEEK_ENDPOINT, {
    ...
    body: JSON.stringify({
      ...
      stream: true,
      messages: [
        { role: 'system', content: '你是一位资深职业规划顾问，精通财务规划与职业发展评估。回复使用中文，语言专业简洁。请结合用户的薪资数据以及主观的岗位前景评估，如果主观前景极好，即使薪资略低也可适当增加推荐权重，最终给出最符合长远职业发展的综合建议。' },
